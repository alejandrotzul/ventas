...
function checkTableName($shortTName, $type=false)
{
	...
	if ("cliente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("detallefactura" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("facturas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("producto" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("producto_has_ventas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function xt_showchart($params)
{
	...
	echo '<div id="' . $chartParams['containerId'] . '" style="width:' . $width . 'px;height:' . $height . 'px"></div>';
...
//	GetLocalLink makes any difference with GetTableLink in ASP.NET only
	...
	if( $globalSettings["LandingPage"] == "login" || $globalSettings["LandingPage"] == "register" )
		return GetLocalLink("menu");
}
