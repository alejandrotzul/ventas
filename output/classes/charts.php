...
class Chart
{
	...
	protected function isProjectDB()
	{
		...
		if("cliente" == $this->chrt_array['tables'][0])
			return true;
		if("detallefactura" == $this->chrt_array['tables'][0])
			return true;
		if("facturas" == $this->chrt_array['tables'][0])
			return true;
		if("producto" == $this->chrt_array['tables'][0])
			return true;
		if("producto_has_ventas" == $this->chrt_array['tables'][0])
			return true;
		if("ventas" == $this->chrt_array['tables'][0])
			return true;
		return false;
	}
	...
	{
		if( $this->showDetails && count( $this->detailTablesData ) && !$this->dashChart ) 
		{
	...
	protected function getSeriesData( $name, $pointsData, $clickData, $seriesNumber )
	{
		...
		if( $this->showDetails && !$this->dashChart ) 
		{
			$data["tooltip"] = array( 
				"enabled" => true,
				"title" => array( 
	...
	protected function setGaugeSpecChartSettings( &$chart, $seriesNumber )
	{
		...
		$chart["chartLabels"] = array( 
			array(
				...
				"hAlign" => "center",
				"offsetY" => -150, //fix it				
				"text" => $this->chart_xmlencode($this->arrDataSeries[0]).": ".$chart["data"][0]["value"], //fix it
